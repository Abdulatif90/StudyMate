...
- **Migration `a1b2c3d4e5f6`** adds the `assignment_submissions` table (+ its
  ...
  single head. **Hand-written**, not autogenerated (the live Neon DB is behind — the whole
  Phase 5 chain is batched to the project-end live pass). Model registered in `alembic/env.py`.
  **NOT applied to Neon** — pending live step, batched with `faccee6a0508` / `5ccf38a52dfb`.
...
- **Migration `faccee6a0508`** adds the `assignments` table (+ indexes on org_id/owner_id/
  ...
  heads` stays SINGLE (`faccee6a0508`). **NOT applied to Neon — pending live step** (do
  not run `alembic upgrade` against Neon; batched with the flashcard revision).
